...
Recrie os containers `app` e `queue` depois de atualizar o código, para que a configuração do proxy seja recarregada. Mantenha `APP_URL=http://localhost:8000`; a URL pública HTTPS é reconhecida pelos cabeçalhos confiáveis do ngrok. Em seguida faça um recarregamento forçado no navegador. O túnel deve servir apenas os arquivos HTTPS de `/build/assets`; não libere `0.0.0.0` nem uma origem HTTP do ngrok na CSP.
