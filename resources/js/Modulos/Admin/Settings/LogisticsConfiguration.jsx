import MelhorEnvioServices from './MelhorEnvioServices';
...
const SenderForm = ({ sender, saving, onChange, onSave }) => (
  <form className="hub-settings-form hub-surface" onSubmit={onSave}>
    ...
    <div className="hub-settings-fields">
      ...
      <label>Inscrição estadual<input maxLength={30} value={sender.inscricao_estadual || ''} onChange={(event) => onChange({ ...sender, inscricao_estadual: event.target.value })} /></label>
      <label>CNAE<input maxLength={10} value={sender.cnae || ''} onChange={(event) => onChange({ ...sender, cnae: event.target.value })} /></label>
...
export const LogisticsConfiguration = ({ defaultEnvironment }) => {
  ...
  const connected = Boolean(settingsQuery.data?.data?.is_authenticated) && connection?.status === 'CONNECTED';
  ...
  return (
    <section className="hub-settings-gateways hub-stable-data-region">
      ...
      <MelhorEnvioServices settings={settingsQuery.data?.data} onSaved={() => settingsQuery.refetch()} />
