...
export const AppGuides = () => {
  ...
  return <section className="hub-app-guides hub-surface">
    ...
    <div className="hub-app-guides-controls"><label><Search size={16} aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar OAuth, PIX, webhook…" /></label><select value={provider} onChange={(event) => setProvider(event.target.value)}>{['Todos', 'Stripe', 'Mercado Pago', 'PagBank', 'Pagar.me'].map((item) => <option key={item}>{item}</option>)}</select></div>
