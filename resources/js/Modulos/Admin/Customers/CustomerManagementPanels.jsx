...
import { errorMessage, formatCurrency } from './customerUtils';
...
const ManagementSkeleton = () => (
  <section className="hub-surface hub-customer-management-loading" role="status" aria-live="polite" aria-label="Carregando configurações de clientes" aria-busy="true">
    <Skeleton />
    <Skeleton />
    <Skeleton />
  </section>
...
const ManagementUnavailable = ({ onRetry }) => (
  <section className="hub-surface hub-error-state" role="alert">
    <div>
      <h2 className="hub-panel-title">Dados indisponíveis</h2>
      <p>Não foi possível carregar este painel de clientes. Nenhuma configuração foi estimada.</p>
      <Button className="mt-5" variant="secondary" onClick={onRetry}>Tentar novamente</Button>
    </div>
  </section>
);

export const CustomerManagementPanels = ({ mode = 'VIP', vipLevels, settings, loading, error, onRetry, onSaveVip, onDeleteVip, onSaveSettings }) => {
  ...
  if (error || (mode === 'CONFIG' && !settings)) return <ManagementUnavailable onRetry={onRetry} />;

  if (mode === 'VIP') return <section className="hub-customer-management"><header className="hub-order-detail-heading"><div><p className="hub-page-eyebrow">Relacionamento</p><h1 className="hub-page-title"><Crown aria-hidden="true" size={28} /> Benefícios VIP</h1><p className="hub-page-description">Defina níveis, limites e benefícios por valor de relacionamento.</p></div><Button icon={Plus} disabled={isOperating} onClick={() => setVip(initialVip)}>Novo nível</Button></header>
