...
import { ChevronLeft, ChevronRight, Image as ImageIcon, Pencil, Plus, Search, X } from 'lucide-react';
...
export const CatalogList = ({ products, categories, filters, pagination, loading, onChange, onClear, onCreate, onEdit }) => {
  const hasFilters = Boolean(filters.search || filters.category !== 'TODAS' || filters.status !== 'TODOS');
  ...
  return <section className="hub-surface hub-orders-list">
    ...
    <div className="hub-catalog-filters"><label className="hub-orders-search"><Search aria-hidden="true" size={17} /><span className="sr-only">Buscar produto</span><input value={filters.search} onChange={(event) => onChange({ search: event.target.value, page: 1 })} placeholder="Buscar nome ou SKU" />{filters.search ? <IconButton icon={X} label="Limpar busca" onClick={() => onChange({ search: '', page: 1 })} /> : null}</label>
      <select aria-label="Filtrar por categoria" value={filters.category} onChange={(event) => onChange({ category: event.target.value, page: 1 })}><option value="TODAS">Todas as categorias</option>{categories.map((category) => <option key={category.id} value={category.nome}>{category.nome}</option>)}</select>
      <select aria-label="Filtrar por status" value={filters.status} onChange={(event) => onChange({ status: event.target.value, page: 1 })}><option value="TODOS">Todos os status</option><option value="ATIVO">Ativos</option><option value="INATIVO">Inativos</option><option value="ESGOTADO">Esgotados</option><option value="ENCOMENDA">Sob encomenda</option></select>
      <select aria-label="Itens por página" value={filters.perPage} onChange={(event) => onChange({ perPage: Number(event.target.value), page: 1 })}><option value="15">15 por página</option><option value="30">30 por página</option><option value="50">50 por página</option></select>
      {hasFilters ? <Button size="sm" variant="ghost" onClick={onClear}>Limpar filtros</Button> : null}
    </div>
    {loading && !products.length ? <div className="hub-orders-loading" aria-label="Carregando produtos"><span /><span /><span /></div> : products.length ? <><DesktopRows products={products} onEdit={onEdit} /><MobileRows products={products} onEdit={onEdit} /></> : <div className="hub-empty-state"><div><ImageIcon aria-hidden="true" size={28} /><h2 className="hub-panel-title">Nenhum produto encontrado</h2><p>Altere os filtros ou comece cadastrando o primeiro item da loja.</p><div className="hub-catalog-actions">{hasFilters ? <Button variant="secondary" onClick={onClear}>Limpar filtros</Button> : null}<Button icon={Plus} onClick={onCreate}>Novo produto</Button></div></div></div>}
