...
const ReviewsPage = () => {
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ id, status }) => await api.put(`/admin/reviews/${id}/status`, { status }),
  ...
  const handleAprovar = (id) => {
    statusMutation.mutate({ id, status: 'aprovado' });
  };

  const handleRejeitar = (id) => {
    statusMutation.mutate({ id, status: 'rejeitado' });
  };
                    ...
                    <tr key={av.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-700 max-w-xs truncate" title={av.comentario}>
                          {av.comentario || <span className="text-gray-400 italic">Sem comentário</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium">
                        <div className="flex justify-end gap-2">
                          {av.status !== 'aprovado' && (
                            <button onClick={() => handleAprovar(av.id)} disabled={statusMutation.isPending} className="text-green-600 hover:text-green-900 bg-green-50 p-2 rounded-md transition-colors disabled:opacity-50" title="Aprovar">
                              <CheckCircle size={18} />
                            </button>
                          )}
                          {av.status !== 'rejeitado' && (
                            <button onClick={() => handleRejeitar(av.id)} disabled={statusMutation.isPending} className="text-yellow-600 hover:text-yellow-900 bg-yellow-50 p-2 rounded-md transition-colors disabled:opacity-50" title="Rejeitar">
                              <XCircle size={18} />
                            </button>
                          )}
                          <button onClick={() => handleExcluir(av.id)} disabled={deleteMutation.isPending} className="text-red-600 hover:text-red-900 bg-red-50 p-2 rounded-md transition-colors disabled:opacity-50" title="Excluir">
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </td>
