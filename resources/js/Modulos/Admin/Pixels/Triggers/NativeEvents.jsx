...
import { nativeEventsList } from '../Shared/PixelConstants';
import { AnimatedToggle, SafeTooltip } from '../Shared/PixelUI';

const NativeEvents = ({ eventosNativos, setNativeEvents, isSaving, onSave, isAllNativosAtivos, onToggleAll }) => (
        ...
        <tbody>
          {nativeEventsList.map((event) => (
            <tr key={event.key}>
              ...
              <td className="text-center"><AnimatedToggle active={Boolean(eventosNativos[event.key])} onChange={(value) => setNativeEvents((current) => ({ ...current, [event.key]: value }))} /></td>
            </tr>
          ))}
        </tbody>
      </table>
