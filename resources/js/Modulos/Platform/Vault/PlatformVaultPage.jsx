...
export const PlatformVaultPage = () => {
  ...
  return <main className="hub-platform-vault">
    ...
    <section className="hub-platform-vault-controls"><label><Search size={16} aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filtrar provedor ou finalidade" /></label><select value={environment} onChange={(event) => setEnvironment(event.target.value)}><option value="ALL">Todos os ambientes</option><option value="SANDBOX">Sandbox</option><option value="PRODUCTION">Produção</option></select><button onClick={load} disabled={loading}><RefreshCw className={loading ? 'hub-spinner' : ''} size={17} />Atualizar</button></section>
    <section className="hub-platform-vault-grid">{visible.map((record) => <article key={record.provider + record.purpose + record.environment}><header><strong>{record.provider}</strong><Badge variant={statusVariant[record.status] || 'neutral'}>{record.status}</Badge></header><dl><div><dt>Finalidade</dt><dd>{record.purpose}</dd></div><div><dt>Ambiente</dt><dd>{record.environment}</dd></div><div><dt>Última rotação</dt><dd>{record.rotated_at ? new Date(record.rotated_at).toLocaleString() : 'Ainda não configurada'}</dd></div></dl><footer><span><ShieldCheck size={16} aria-hidden="true" /> Metadados protegidos</span>{record.status !== 'REVOKED' ? <button onClick={() => revoke(record)} disabled={revoking === record.id}>{revoking === record.id ? 'Revogando…' : 'Revogar'}</button> : null}</footer></article>)}</section>
  </main>;
};
