...
- Evidências: `npm run test:ui` passou (3 arquivos/4 testes Vitest e 6 testes de interação); `npm run build` passou e a busca em `public/build` não encontrou `localhost:8000/api`.
- Riscos, bloqueios e itens não verificados: `composer` e `docker` não estão disponíveis no ambiente de trabalho, portanto o teste PHP novo e a homologação com ngrok permanecem pendentes. `npm run lint` continua vermelho por 51 erros preexistentes em módulos legados, sem arquivos tocados neste commit.
- Próxima ação única: abrir PR, executar os gates PHP/CI e aplicar os comandos documentados no host do túnel antes de validar o navegador.
