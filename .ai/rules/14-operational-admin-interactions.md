...
- Documentos: listar arquivos reais de pagamento, entrega/romaneio, reembolso, declaração, XML e DANFE. Estado vazio só aparece quando não existem documentos. Todo novo comprovante operacional usa storage privado tenant-scoped e URL temporária assinada para prévia/download; arquivo legado público é sinalizado para migração, nunca reexposto por URL pública. Nenhum espelho HTML deve ser anunciado como NF-e emitida.
- Totais: expor subtotal, bruto, desconto total, frete cobrado/custo e líquido somente conforme contrato. Cupons persistidos podem ser listados com sua origem; VIP, loja e frete não podem receber valor zero ou rótulo inferido quando não houver snapshot. Cupom antecede VIP; benefícios de frete/produto possuem limites separados e não se transferem entre bases. Não inferir lucro, custo ou bandeira de cartão inexistente. Detalhamento final exige snapshot financeiro persistido e cálculo em centavos/decimal no servidor.
...
- Centro de Apps: Configurações mostra catálogo e instalações por tenant. Logística abre a Central de Logística & Envios, sem duplicar transportadoras, remetente ou embalagens em Configurações.\n- Fiscal: o App Fiscal guarda emitente, certificado A1/PFX e credenciais criptografados por tenant; a resposta nunca contém senha, token ou conteúdo do certificado. O preflight consulta o catálogo ativo e aponta pendências de NCM, origem e CFOP sem inventar dados. Declaração de conteúdo só usa remetente/destinatário completos e sempre informa que não substitui NF-e. Emissão exige dados fiscais do pedido, adapter real e homologação; consultar `docs/architecture/orders-fiscal-readiness.md`.
