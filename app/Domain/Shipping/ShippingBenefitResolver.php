...
final class ShippingBenefitResolver
{
    ...
    public function benefitsFor(array $pricedItems, int $shippingCents): array
    {
        ...
        $productIds = collect($pricedItems)->map(
            static fn (array $item): int => (int) $item['product']->getKey(),
        )->unique()->values();
    ...
    {
        return $productId > 0 && collect($pricedItems)->every(
            static fn (array $item): bool => (int) $item['product']->getKey() === $productId,
        );
    }
