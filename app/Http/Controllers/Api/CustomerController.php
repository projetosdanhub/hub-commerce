...
class CustomerController extends Controller
{
    ...
    {
        // 🔒 Filtra APENAS quem é cliente e carrega relacionamentos
        ...
        $formatted = $users->map(function ($c) {
            ...
            return [
                ...
                'reembolsosPagos' => (float) $pedidosReembolsados->sum('total'),
                'enderecos' => $c->addresses,

                // HISTÓRICO VISUAL DE PEDIDOS (Nova Aba do CRM)
                'pedidos' => $c->orders->map(function ($order) {
                    $cupons = is_string($order->applied_coupons) ? json_decode($order->applied_coupons, true) : $order->applied_coupons;
                    return [
                        'id' => $order->id,
                        'status' => $order->status,
    ...
    {
        $cliente = User::with(['addresses', 'orders.items', 'orders.history', 'auditLogs' => function($q) {
            $q->orderBy('created_at', 'desc');
        }])->findOrFail($id);
    ...
    {
        $receitaBruta = Order::where('status', '!=', 'CANCELADO')->sum('total');
        $totalPedidos = Order::where('status', '!=', 'CANCELADO')->count();
        $ticketMedio  = $totalPedidos > 0 ? ($receitaBruta / $totalPedidos) : 0;

        $receitaMesAtual = Order::where('status', '!=', 'CANCELADO')
                                 ->whereMonth('created_at', now()->month)
                                 ->whereYear('created_at', now()->year)
                                 ->sum('total');

        $receitaMesAnterior = Order::where('status', '!=', 'CANCELADO')
        ...
        $pedidosMesAtualCount = Order::where('status', '!=', 'CANCELADO')
                                 ->whereMonth('created_at', now()->month)
                                 ->whereYear('created_at', now()->year)
                                 ->count();
        
        $pedidosMesAnteriorCount = Order::where('status', '!=', 'CANCELADO')
                                    ->whereMonth('created_at', now()->subMonth()->month)
                                    ->whereYear('created_at', now()->subMonth()->year)
                                    ->count();
    ...
    // =========================================================================
    // 11. REGRAS E NÍVEIS VIP (Com Suporte a Upload de Badges)
    // =========================================================================
