...
| [~] | UI-026 | P1 | Detalhar valores e progresso operacional | UI-021 | Em andamento: resumo usa subtotal/frete/desconto/total e cupons persistidos; não atribui VIP/loja/frete sem snapshot. Ícone usa método confirmado, bandeira só com metadado real; trilha animada, ambos os temas/mobile. Base criada: snapshot financeiro versionado em centavos, benefícios por base, precificação server-side e contrato tenant-scoped de cotação opaca com fingerprint/expiração; adapter de cálculo e persistência de opções opacas iniciados, faltam endpoint público de cotação a partir do carrinho, payment_attempt e integração atômica do checkout |
