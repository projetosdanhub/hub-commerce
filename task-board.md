...
| Status | ID | Prioridade | Tarefa | Dependência | Critério de aceite |
|---|---|---:|---|---|---|
| [~] | CRM-001 | P1 | Isolar cadastro de cliente por tenant | TEN-008 | Auditoria da UI de Clientes identificou consulta global de `User`, pedidos, endereços, métricas, VIP e configurações; a interface foi colocada em estado indisponível seguro na branch `ui/customers-modernization`. Modelar vínculo por tenant antes de liberar qualquer aba |
...
| [~] | UI-017 | P1 | Aplicar padrão canônico ao módulo Clientes/CRM | UI-005, UI-006, UI-007, UI-009 | O módulo agora preserva as abas em composição tokenizada, acessível, clara/escura e mobile, mas não exibe métricas, diretório, perfil, VIP ou configurações enquanto CRM-001 não isolar os contratos por tenant |
